...
public interface BookingsRepository extends JpaRepository<Bookings,Integer> {
    ...
    @Query("SELECT t.name, t.number, b.bookingDate, b.travelDate, fs.name, ts.name, b.numberOfTickets, b.seatType, b.amountPaid, b.id, u.id FROM Bookings b " +
            "JOIN User u ON u.id = b.user.id " +
            "JOIN Train t ON t.id = b.train.id " +
            "JOIN Station ts ON ts.id = b.tostation.id " +
            "JOIN Station fs ON fs.id = b.fromstation.id " +
            "WHERE b.user.id = :userId " +
            "ORDER BY b.travelDate DESC")
    List<Object[]> getBookingsByUserId(int userId);

    @Query("SELECT t.name, t.number, b.bookingDate, b.travelDate, fs.name, ts.name, b.numberOfTickets, b.seatType, b.amountPaid, b.id, u.id  " +
            "FROM Bookings b " +
            "JOIN b.user u " +
            "JOIN b.train t " +
            "JOIN b.tostation ts " +
            "JOIN b.fromstation fs " +
